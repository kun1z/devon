...
//----------------------------------------------------------------------------------------------------------------------
static void inner_wp_hash(uint64_t K[2][8], uint64_t state[2][8], uint64_t const * const restrict rc)
{
    for (int i=0;i<10;i++)
    {
...
void whirlpool_mod2(void * const out64, void const * const in64, void const * const in64_2, void const * const key80)
{
    ...
    if (key80)
    {
        for (int i=0;i<80;i++)
        {
            unsigned char const * const k = key80;
            unsigned char * const iv = (unsigned char*)rc;
            iv[i] ^= k[i];
        }
    }

    inner_wp_hash(K, state, rc);

    for (int i=0;i<8;i++)
    {
        unsigned char const * const restrict p = in64;
        uint64_t in_q;
        memcpy(&in_q, &p[i * 8], 8);
